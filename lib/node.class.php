...
class node  {
  ...
  public static function post($module, $url, $options, $data) {
    ...
    return ['status' => curl_getinfo($handler, CURLINFO_HTTP_CODE), 'data' => $data, 'benchmark' => (microtime(true)-$before)];

  }

  public static function checkProcess() {
    exec('pgrep -xlf "node srv/node_web.js"', $output);
    if (count($output) < 1) {
      return false;
    }
    return true;
  ...
  public static function startProcess() {
    exec('node srv/node_web.js > /dev/null 2>&1 &', $output, $return);
    //exec('node srv/node_web.js > /dev/null 2>&1 &', $output, $return);
    sleep(1);
  }
